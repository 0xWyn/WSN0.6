...
import XMark from "../../../components/icons/x-mark.jsx";
export default function ClanPage() {
    ...
    return (
        <div>
            {showCreationModal && (
                <div className="fixed w-full h-full bg-black/10 z-20 inset-0 backdrop-blur-md flex items-center justify-center">
            ...
            <div className="w-full sticky flex justify-between pr-4">
                <LocNav current={clan?.name} />

                <button
                    onClick={() => setShowCreationModal((prev) => !prev)}
                    className="h-10 w-10 p-2 rounded-2xl flex items-center justify-center bg-slate-100 hover:border-2 transition duration-300"
                >
                    <Pencil />
                </button>
            </div>
